...
function asingDayStyle(FirstClass,details,elementClass)
{
    ...
    details.forEach(obj => 
        ...
        allDays.forEach(dia => 
            {
                let mensajetotalPago = document.createElement('span')   
                mensajetotalPago.classList.add('mensaje-total-Pago')
                mensajetotalPago.innerText = `(${dia.id}) ${obj[1].dinero.valor.toLocaleString("en-US", {style:"currency", currency:"COP"})}`
                
                if(dia.id  == obj[0] && obj[1].estado.id == 2)
                {
                    dia.classList.add(`${elementClass}-${obj[1].estado.descripcion}`)
                    dia.innerText = '✓'
                    dia.appendChild( mensajetotalPago)
                }
